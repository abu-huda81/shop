...
def create_product(
    db: Session, name: str, description: str, price: float, image_files: List[str]
):
    ...
    for file in image_files:
        ...
        image = ProductImage(
            ...
            image_url=f"/static/product_images/{product.id}/{file.filename}",
        )
        db.add(image)
...
def get_products(db: Session):
    ...
    for product in products:
        product.image_urls = [
            os.path.join(BASE_URL, image.image_url) for image in product.images
        ]
    return products
